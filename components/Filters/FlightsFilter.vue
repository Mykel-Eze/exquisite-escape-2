<template>
  <div class="filter-blocks-wrapper">
    ...
    <div class="filter-block">
      <div class="filter-block-header">
        <span>Stops</span>
        <small class="pry-color clear-btn" @click="clear(selectedStops, stops)"
          >Clear</small
        >
      </div>
        ...
        <label v-for="(stop, index) in stops" :key="index" :for="stop.item">
          ...
          <span class="w-full">
            <span class="flex-div justify-between">
              ...
              <span class="filter-block-item-value"
                >₦{{ formatNumber(stop.value) }}</span
              >
            </span>
          </span>
        </label>
    ...
    <div class="filter-block">
      <div class="filter-block-header">
        <span>Airline</span>
        <small
          class="pry-color clear-btn"
          @click="clear(selectedAirlines, airlines)"
          >Clear</small
        >
      </div>
      <div class="filter-list-items">
        <label
          v-for="(airline, index) in displayedAirlines"
          :key="index"
          :for="airline.name"
        >
          <input
          ...
          <span class="w-full">
            <span class="flex-div justify-between">
              <span class="filter-block-item truncate"
                >({{ airline.number }}) - {{ airline.name }}</span
              >
              <span class="filter-block-item-value"
                >₦{{ formatNumber(airline.value) }}</span
              >
            </span>
          </span>
        </label>
      ...
      <div class="show-more-btn-wrapper mt-[15px]">
        <button
          v-if="airlines.length > 10"
          @click="showMore = !showMore"
          class="flex-div gap-2 bold-txt pry-color text-[14px]"
        >
    ...
    <div class="filter-block">
      <div class="filter-block-header">
        ...
        <small
          class="pry-color clear-btn"
          @click="clear(selectedBaggages, baggages)"
          >Clear</small
        >
      </div>
      <div class="filter-list-items">
        <label
          v-for="(baggage, index) in baggages"
          :key="index"
          :for="baggage.item"
        >
          <input
          ...
          <span class="w-full">
            <span class="flex-div justify-between">
              ...
              <span class="filter-block-item-value"
                >₦{{ formatNumber(baggage.value) }}</span
              >
            </span>
          </span>
        </label>
    ...
    <div class="filter-block">
      <div class="filter-block-header">
        ...
        <small
          class="pry-color clear-btn"
          @click="clear(selectedDepartures, departures)"
          >Clear</small
        >
      </div>
      <div class="filter-list-items">
        ...
        >
          ...
          <span class="w-full">
            <span class="flex-div justify-between">
              ...
              <span class="filter-block-item-value"
                >{{ departure.startTime }} - {{ departure.endTime }}</span
              >
            </span>
          </span>
        </label>
    ...
    <div class="filter-block">
      <div class="filter-block-header">
        ...
        <small
          class="pry-color clear-btn"
          @click="clear(selectedArrivals, arrivals)"
          >Clear</small
        >
      </div>
      <div class="filter-list-items">
        <label
          ...
          :key="index"
          :for="arrival.time + index"
        >
          <input
            type="checkbox"
            :id="arrival.time + index"
            v-model="selectedArrivals"
            :value="arrival.time"
            class="filled-in"
          />
          <span class="w-full">
            <span class="flex-div justify-between">
              ...
              <span class="filter-block-item-value"
                >{{ arrival.startTime }} - {{ arrival.endTime }}</span
              >
            </span>
          </span>
        </label>
...
onMounted(() => {
  ...
  if (slider) {
    let sliderInstance = noUiSlider.create(slider, {
